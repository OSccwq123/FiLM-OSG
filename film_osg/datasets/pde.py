...
class pde_dataset_osg():
    ...
    def load(self, file_path_train, file_path_test):
        ...
        if self.problem_type in ["1d_regular", "1d_irregular", "2d_irregular"]:
            target_dt = np.tile(target_dt[:, np.newaxis, :], [1, L, 1])
            target_X = np.concatenate((target_X, target_dt), axis=-1)
        else:
            target_dt = np.tile(target_dt[:, np.newaxis, np.newaxis, :], [1, H, W, 1])
            target_X = np.concatenate((target_X, target_dt), axis=-1)

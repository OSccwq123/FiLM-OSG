...
def ode_evaluate(prediction, truth, save_path, dt=None):
    """
    ...
    if dt is not None:
        steps = np.zeros(prediction.shape[2])
        steps[1:] = np.cumsum(dt[-1, :])
    else:
        steps = np.arange(prediction.shape[2])

    if truth is None:
        for i in range(dim):
            plt.figure(figsize=(9,9), dpi=300)
            plt.plot(steps, prediction[-1,i,:], linestyle="dashed", color="blue", label="pred")
            plt.legend()
        ...
        plt.figure(figsize=(9,9), dpi=300)
        for i in range(dim):
            plt.plot(steps, rel_err[i,:], label=r"$u_{{{}}}$".format(i+1))
        plt.legend()
        plt.savefig(save_path+"/rel_err.png")
        plt.close()
        ...
        for i in range(dim):
            plt.figure(figsize=(9,9), dpi=300)
            plt.plot(steps, truth[-1,i,:], color="red", label="true")
            plt.plot(steps, prediction[-1,i,:], linestyle="dashed", color="blue", label="pred")
            plt.legend()
            plt.savefig(save_path+"/pred_{}.png".format(i))
            plt.close()

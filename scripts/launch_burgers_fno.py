#!/usr/bin/env python3
"""Run Burgers training jobs across a list of GPUs."""
...
TRAIN_SCRIPT = REPO_ROOT / "train" / "run_burgers_fno.py"
...
def training_command(args, model, seed):
    ...
    if model == "gl_fno_film":
        command.extend(["--gl-film-mode", args.gl_film_mode])
    if args.hf_weight:
...
def parse_args():
    ...
    parser.add_argument("--dataset", choices=("original", "sharp"), default="original")
    parser.add_argument("--data-dir", default="data")
    parser.add_argument("--save-dir", default=".")
    parser.add_argument("--log-dir", default="logs_burgers_fno")
    ...
    parser.add_argument("--gl-film-mode", choices=("branchwise", "global_only"), default="branchwise")
...
def main():
    ...
    print(
        f"Launching {len(jobs)} Burgers jobs: models={models}, "
    ...
    while jobs or running:
        for gpu in gpus:
            ...
            log_path = log_dir / f"burgers_{model}_seed{seed}{tag}_gpu{gpu}.log"
            log_file = log_path.open("w", buffering=1)
            env = os.environ.copy()
            env["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"
            env["CUDA_VISIBLE_DEVICES"] = str(gpu)

            command = training_command(args, model, seed)
    ...
    print("All Burgers jobs finished successfully.")

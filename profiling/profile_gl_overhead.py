...
def rel_l2_loss(pred, target, eps=1e-12):
    ...
    denominator = torch.linalg.norm(target_flat, dim=1) + eps
...
def training_step(model, xb, yb, opt, tmin, tmax, multiscale, loss_kind, sg_weight):
    ...
    x0, dt1_norm, dt2_norm, dt12_norm = make_sg_batch(xb, tmin, tmax, multiscale)
    ...
    lsg = 0.5 * (
        data_loss(pred012, pred2, loss_kind)
        + data_loss(pred021, pred2, loss_kind)
    )
...
def build(case, model, vmin, vmax, tmin, tmax, config):
    from film_osg.networks.fno import osg_fno1d_with_film, gl_osg_fno1d_with_film
...
def training_path(case, data_dir):
    if case == 'burgers_sharp':
        return str(Path(data_dir) / 'BurgersSharpOSG_train.mat')
    raise ValueError(case)


def profile(case, model_name, batch_size, seed, warmup, iters, device, data_dir):
    from film_osg.datasets.pde import pde_dataset_osg
    set_seed(seed)
    cfg = make_config(case, model_name, batch_size, seed)
    tr = training_path(case, data_dir)
    ds = pde_dataset_osg(cfg)
    trainX, trainY, _, _, vmin, vmax, tmin, tmax, _, _ = ds.load(tr, None)
...
def main():
    ...
    ap.add_argument("--cases", default="burgers_sharp")
    ap.add_argument("--models", default="film,gl_film")
    ap.add_argument("--warmup", type=int, default=10)
    ap.add_argument("--iters", type=int, default=50)
    ap.add_argument("--seed", type=int, default=0)
    ap.add_argument("--device", default="cuda")
    ap.add_argument("--data-dir", default="data/burgers_sharp")
    ap.add_argument("--save-dir", default="overhead_outputs_gl")
    args = ap.parse_args()
    os.makedirs(args.save_dir, exist_ok=True)
    rows = []
    for case in [value for value in args.cases.split(",") if value]:
        batch_size = 20 if case == "ns" else 100
        for model in [value for value in args.models.split(",") if value]:
            print(f"Profiling {case} {model} bs={batch_size}", flush=True)
            row = profile(
                case,
                model,
                batch_size,

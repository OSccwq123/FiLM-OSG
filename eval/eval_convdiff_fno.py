...
def model_path_for(model_name, seed, tag, root="."):
    ...
    return Path(root) / f"runs_convdiff_{model_name}_seed{seed}{suffix}" / "model"


def compute_metrics(pred, truth, eps=1e-12):
    """Compute rollout errors, excluding the supplied initial state."""
    pred_roll = pred[..., 1:]
    true_roll = truth[..., 1:]
...
def flatten_summary_rows(summary_by_model, metric_keys):
    ...
    for model_name, item in summary_by_model.items():
        ...
        for key in metric_keys:
            row[f"{key}_mean"] = item["metrics"][key]["mean"]
            row[f"{key}_std"] = item["metrics"][key]["std"]
        rows.append(row)
    return rows
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate advection--diffusion rollouts across trained seeds.")
    ...
    print(f"Advection--diffusion evaluation: models={models}, seeds={seeds}")
    ...
    write_csv(save_dir / "convdiff_fno_seedwise.csv", seedwise_rows)
    write_csv(
        save_dir / "convdiff_fno_summary_by_model.csv",
        flatten_summary_rows(summary_by_model, metric_keys),
    )
    write_csv(save_dir / "convdiff_fno_paired_summary.csv", paired_summary)

...
before pairs with varying evolution times are sampled.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--min-time-offset-steps", type=int, default=1)
    parser.add_argument("--max-time-offset-steps", type=int, default=20)
...
def sample_pairs(
    ...
    if not 1 <= min_time_offset <= max_time_offset < n_time:
        raise ValueError(
            f"Invalid time-offset range [{min_time_offset},{max_time_offset}] for T={n_time}."
        )

    sample = read_frame(first, 0, stride)
    trajectories = np.empty((n_pairs, *sample.shape, 2), dtype=sample.dtype)
    time_intervals = np.empty((n_pairs, 1), dtype=np.float64)
    ...
    for pair_id, group_key in enumerate(balanced_schedule(group_keys, n_pairs, rng)):
        ...
        offset_steps = int(rng.integers(min_time_offset, max_time_offset + 1))
        start = int(rng.integers(0, n_time - offset_steps))
        stop = start + offset_steps
        trajectories[pair_id, ..., 0] = read_frame(data, start, stride)
        trajectories[pair_id, ..., 1] = read_frame(data, stop, stride)
        time_intervals[pair_id, 0] = times[stop] - times[start]

    if not np.all(time_intervals > 0):
        raise ValueError("PDEBench time coordinates must be strictly increasing.")
    return trajectories, time_intervals
...
def write_split(
    ...
    savemat(
        path,
        {
            ...
            "dt": time_intervals.astype(dtype, copy=False),
...
def main() -> None:
    ...
    with h5py.File(args.input, "r") as h5:
        ...
        train, train_dt = sample_pairs(
            ...
            train_groups,
            args.train_pairs,
            args.min_time_offset_steps,
            args.max_time_offset_steps,
            args.space_stride,
            args.data_key,

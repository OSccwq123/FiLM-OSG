...
- `scripts/data_generation/convert_pdebench_to_osg.py`: samples pair-level
  variable-lag `.mat` files from local PDEBench HDF5 trajectories.
...
conversion commands and their pair-level split limitation are documented in
the repository README; no PDEBench data are tracked by git.
